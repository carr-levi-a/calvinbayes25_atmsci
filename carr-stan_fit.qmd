...
generated quantities {
 // this is not really required - only if you want values of omega, alpha, beta included in your fitted model
  ...
  vector[n] lambda;
  for (i in 1:n) {
    mu[i] = exp(b0 + b1[ice_idx[i]]); // if you want stan to compute expected values for each data row
    alpha[i] = mu[i] / sigma^2; // if you want the beta parameters alpha and lambda for each row
    lambda[i] = mu[i]^2 / sigma^2;
  }
}'
```
...
```
```{r Model Fit}
cirrus_fit_1 <- rstan::stan(model_code = stan_cirrus_reff_1,
                  data = stan_cirrus_data,
                  chains = 1, 
                  iter = 500)

```
```{R Save local copy}
model_file <- "cirrus_fit_1.RDS"
saveRDS(cirrus_fit_1, file = model_file)
```

```{r Check Convergence}
#cirrus_fit_1
#mcmc_rank_overlay(cirrus_fit_1, pars=c(b0, b1[1],b1[2],b1[3]))
mcmc_trace(cirrus_fit_1, pars=c('b0', 'b1[1]','b1[2]','b1[3]'))
```
