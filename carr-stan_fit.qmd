...
model {
  ...
  b1[1] ~ normal(-0.04445, 0.2);
  b1[2] ~ normal(0.0000, 0.4);
  b1[3] ~ normal(0.04256, 0.2);
...
```{r Model Fit}
refit_model <- TRUE
model_file <- "cirrus_fit_mixed_phase.RDS"

if (refit_model){
  cirrus_fit <- stan(model_code = stan_cirrus_reff,
...
```{r Check Convergence}
summary(cirrus_fit, pars=c('b0', 'b1[1]','b1[2]','b1[3]'))
