...
gp <- CalvinBayes::gamma_params(mean=base::log(20), sd=base::log(15))
shpe <- as.numeric(gp[1])
rte <- as.numeric(gp[2])

prior_pred_dist <- tibble(sim_id = c(1:n_sim)) |> 
   mutate(b0 = rcauchy(n_sim, log(20), log(20)),
          b1 = rcauchy(n_sim, 1, 2),
...
```
```{r}
gf_dist(dist="cauchy", params=c(location=shpe, scale=rte)) |>
  gf_lims(x=c(-10,10))
```
...
gf_point(sim_Reff ~ cf_ice.z | sim_id,
         alpha = 0.7,
         data = prior_pred_dist |>
          filter(sim_id < 31) # show just the first 30 sim datasets
        ) |> gf_lims(y = c(0,100))
