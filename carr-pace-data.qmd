...
gp <- CalvinBayes::gamma_params(mean=log(50), sd = log(15))

prior_pred_dist <- tibble(sim_id = c(1:n_sim)) |> 
   mutate(b0 = rgamma(n_sim, log(20), log(10)),
          b1 = rcauchy(n_sim, 1, 2.5),
          b2 = rcauchy(n_sim, -2, 2.5),
          sigma = rlnorm(n_sim, 0, 1)
          ) |>
   rowwise() |>
   mutate(mu = list(exp(b0 + b1*cloud_data$cf.z + b2*cloud_data$alb.z)),
          cf = list(cloud_data$cf),
          cf.z = list(cloud_data$cf.z),
          alb = list(cloud_data$alb),
          alb.z = list(cloud_data$alb.z)
          ) |>
   unnest(cols =  c(mu, cf, cf.z, alb, alb.z) ) |>
   ungroup() |>
