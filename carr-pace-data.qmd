...
gf_histogram(~cf_ice.z, data = cloud_data)
gf_histogram(~alb, data = cloud_data)
```

categorical variable for liquid cloud or ice cloud?
...
prior_pred_dist <- tibble(sim_id = c(1:n_sim)) |> 
   mutate(b0 = rnorm(n_sim, log(20), log(exp(1))),
        ...
        #  b2 = rcauchy(n_sim, -1, 2),
          sigma = rlnorm(n_sim, log(20), 0.7)
          ) |>
   rowwise() |>
   mutate(mu = list(exp( b0 ) ), # b1*cloud_data$cf_ice.z + b2*cloud_data$alb.z
          cf.z = list(cloud_data$cf.z),
          cf_ice.z = list(cloud_data$cf_ice.z),
          alb = list(cloud_data$alb),
          alb.z = list(cloud_data$alb.z)
          ) |>
   unnest(cols =  c(mu, cf.z, cf_ice.z, alb, alb.z) ) |>
   ungroup() |>
   mutate(alpha = mu^2 / sigma^2,
          lambda = mu / sigma^2
   ) |>
   rowwise() |>
   mutate( sim_Reff = rgamma(1, rate=lambda, shape = alpha) ) |>
   ungroup()
```
```{r}
gf_dist(dist="lnorm", params=c(log(20), 0.7))
```
...
gf_point(sim_Reff ~ cf_ice.z | sim_id,
         alpha = 0.02,
         data = prior_pred_dist |>
          filter(sim_id < 11) # show just the first 30 sim datasets
        ) |> gf_lims(y = c(0,100))
